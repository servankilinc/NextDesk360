...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectMovementTypeService : IProjectMovementTypeService
    {
        ...
        {
            var affected = await _unitOfWork.ProjectMovementTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.ProjectMovementTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
