...
namespace ExpressDesk360.Business.Concrete
{
    public class WarehouseService : IWarehouseService
    {
        ...
        {
            await _unitOfWork.Warehouses.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.Warehouses.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
