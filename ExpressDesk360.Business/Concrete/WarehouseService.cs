...
namespace ExpressDesk360.Business.Concrete
{
    public class WarehouseService : IWarehouseService
    {
        ...
        {
            var affected = await _unitOfWork.Warehouses.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Warehouses.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
