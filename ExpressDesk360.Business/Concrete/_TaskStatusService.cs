...
namespace ExpressDesk360.Business.Concrete
{
    public class _TaskStatusService : I_TaskStatusService
    {
        ...
        {
            await _unitOfWork._TaskStatuses.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork._TaskStatuses.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
