...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectMovementService : IProjectMovementService
    {
        ...
        {
            await _unitOfWork.ProjectMovements.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.ProjectMovements.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
