...
namespace ExpressDesk360.Business.Concrete
{
    public class StockTypeService : IStockTypeService
    {
        ...
        {
            await _unitOfWork.StockTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
