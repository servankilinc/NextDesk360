...
namespace ExpressDesk360.Business.Concrete
{
    public class StockTypeService : IStockTypeService
    {
        ...
        {
            var affected = await _unitOfWork.StockTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.StockTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
