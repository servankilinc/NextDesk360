...
namespace ExpressDesk360.Business.Concrete
{
    public class StockBrandService : IStockBrandService
    {
        ...
        {
            await _unitOfWork.StockBrands.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockBrands.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
