...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyContactService : ICompanyContactService
    {
        ...
        {
            await _unitOfWork.CompanyContacts.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CompanyContacts.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
