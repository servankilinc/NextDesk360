...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketFileService : ITicketFileService
    {
        ...
        {
            await _unitOfWork.TicketFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.TicketFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
