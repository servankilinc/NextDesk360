...
namespace ExpressDesk360.Business.Concrete
{
    public class BOMItemService : IBOMItemService
    {
        ...
        {
            var affected = await _unitOfWork.BOMItems.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.BOMItems.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
