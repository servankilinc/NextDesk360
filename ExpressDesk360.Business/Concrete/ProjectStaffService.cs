...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectStaffService : IProjectStaffService
    {
        ...
        {
            await _unitOfWork.ProjectStaffs.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.ProjectStaffs.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
