...
namespace ExpressDesk360.Business.Concrete
{
    public class InvoiceService : IInvoiceService
    {
        ...
        {
            var affected = await _unitOfWork.Invoices.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Invoices.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
