...
namespace ExpressDesk360.Business.Concrete
{
    public class _TaskMovementTypeService : I_TaskMovementTypeService
    {
        ...
        {
            await _unitOfWork._TaskMovementTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork._TaskMovementTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
