...
namespace ExpressDesk360.Business.Concrete
{
    public class StockSerialService : IStockSerialService
    {
        ...
        {
            await _unitOfWork.StockSerials.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockSerials.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
