...
namespace ExpressDesk360.Business.Concrete
{
    public class StockSerialService : IStockSerialService
    {
        ...
        {
            var affected = await _unitOfWork.StockSerials.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.StockSerials.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
