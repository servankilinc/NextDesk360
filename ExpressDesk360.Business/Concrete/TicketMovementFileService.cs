...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketMovementFileService : ITicketMovementFileService
    {
        ...
        {
            await _unitOfWork.TicketMovementFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.TicketMovementFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
