...
namespace ExpressDesk360.Business.Concrete
{
    public class WarrantyTypeService : IWarrantyTypeService
    {
        ...
        {
            await _unitOfWork.WarrantyTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.WarrantyTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
