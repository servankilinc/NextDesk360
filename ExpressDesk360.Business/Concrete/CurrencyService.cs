...
namespace ExpressDesk360.Business.Concrete
{
    public class CurrencyService : ICurrencyService
    {
        ...
        {
            var affected = await _unitOfWork.Currencies.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Currencies.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
