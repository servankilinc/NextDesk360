...
namespace ExpressDesk360.Business.Concrete
{
    public class CurrencyService : ICurrencyService
    {
        ...
        {
            await _unitOfWork.Currencies.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.Currencies.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
