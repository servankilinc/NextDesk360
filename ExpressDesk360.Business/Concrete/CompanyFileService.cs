...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyFileService : ICompanyFileService
    {
        ...
        {
            await _unitOfWork.CompanyFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CompanyFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
