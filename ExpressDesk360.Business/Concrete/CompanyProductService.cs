...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyProductService : ICompanyProductService
    {
        ...
        {
            await _unitOfWork.CompanyProducts.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CompanyProducts.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
