...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyProductWarrantyService : ICompanyProductWarrantyService
    {
        ...
        {
            await _unitOfWork.CompanyProductWarranties.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CompanyProductWarranties.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
