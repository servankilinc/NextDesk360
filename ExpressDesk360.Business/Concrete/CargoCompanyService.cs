...
namespace ExpressDesk360.Business.Concrete
{
    public class CargoCompanyService : ICargoCompanyService
    {
        ...
        {
            var affected = await _unitOfWork.CargoCompanies.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.CargoCompanies.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
