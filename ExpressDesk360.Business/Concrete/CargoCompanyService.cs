...
namespace ExpressDesk360.Business.Concrete
{
    public class CargoCompanyService : ICargoCompanyService
    {
        ...
        {
            await _unitOfWork.CargoCompanies.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CargoCompanies.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
