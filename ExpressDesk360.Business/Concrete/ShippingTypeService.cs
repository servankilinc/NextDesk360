...
namespace ExpressDesk360.Business.Concrete
{
    public class ShippingTypeService : IShippingTypeService
    {
        ...
        {
            var affected = await _unitOfWork.ShippingTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.ShippingTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
