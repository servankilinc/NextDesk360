...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectFileService : IProjectFileService
    {
        ...
        {
            var affected = await _unitOfWork.ProjectFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.ProjectFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
