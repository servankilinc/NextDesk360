...
namespace ExpressDesk360.Business.Concrete
{
    public class ShippingService : IShippingService
    {
        ...
        {
            var affected = await _unitOfWork.Shippings.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Shippings.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
