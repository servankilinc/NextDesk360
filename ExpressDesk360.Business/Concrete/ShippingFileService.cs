...
namespace ExpressDesk360.Business.Concrete
{
    public class ShippingFileService : IShippingFileService
    {
        ...
        {
            await _unitOfWork.ShippingFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.ShippingFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
