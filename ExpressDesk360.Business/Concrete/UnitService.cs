...
namespace ExpressDesk360.Business.Concrete
{
    public class UnitService : IUnitService
    {
        ...
        {
            var affected = await _unitOfWork.Units.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Units.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
