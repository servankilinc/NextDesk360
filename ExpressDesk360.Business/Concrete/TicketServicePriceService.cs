...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketServicePriceService : ITicketServicePriceService
    {
        ...
        {
            await _unitOfWork.TicketServicePrices.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.TicketServicePrices.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
