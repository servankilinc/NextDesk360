...
namespace ExpressDesk360.Business.Concrete
{
    public class StockGroupService : IStockGroupService
    {
        ...
        {
            await _unitOfWork.StockGroups.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockGroups.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
