...
namespace ExpressDesk360.Business.Concrete
{
    public class _TaskMovementService : I_TaskMovementService
    {
        ...
        {
            await _unitOfWork._TaskMovements.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork._TaskMovements.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
