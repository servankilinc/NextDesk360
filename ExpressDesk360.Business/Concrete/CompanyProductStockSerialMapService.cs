...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyProductStockSerialMapService : ICompanyProductStockSerialMapService
    {
        ...
        {
            await _unitOfWork.CompanyProductStockSerialMaps.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.CompanyProductStockSerialMaps.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
