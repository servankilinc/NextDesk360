...
namespace ExpressDesk360.Business.Concrete
{
    public class FSFolderService : IFSFolderService
    {
        ...
        {
            await _unitOfWork.FSFolders.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.FSFolders.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
