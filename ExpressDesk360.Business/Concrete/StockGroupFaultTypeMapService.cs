...
namespace ExpressDesk360.Business.Concrete
{
    public class StockGroupFaultTypeMapService : IStockGroupFaultTypeMapService
    {
        ...
        {
            await _unitOfWork.StockGroupFaultTypeMaps.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockGroupFaultTypeMaps.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
