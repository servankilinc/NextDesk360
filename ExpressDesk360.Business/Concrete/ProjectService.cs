...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectService : IProjectService
    {
        ...
        {
            var affected = await _unitOfWork.Projects.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Projects.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
