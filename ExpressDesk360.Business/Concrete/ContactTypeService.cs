...
namespace ExpressDesk360.Business.Concrete
{
    public class ContactTypeService : IContactTypeService
    {
        ...
        {
            await _unitOfWork.ContactTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.ContactTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
