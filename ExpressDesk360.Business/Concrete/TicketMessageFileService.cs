...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketMessageFileService : ITicketMessageFileService
    {
        ...
        {
            var affected = await _unitOfWork.TicketMessageFiles.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.TicketMessageFiles.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
