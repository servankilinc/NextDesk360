...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketStaffService : ITicketStaffService
    {
        ...
        {
            var affected = await _unitOfWork.TicketStaffs.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.TicketStaffs.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
