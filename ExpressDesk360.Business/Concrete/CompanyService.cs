...
namespace ExpressDesk360.Business.Concrete
{
    public class CompanyService : ICompanyService
    {
        ...
        {
            var affected = await _unitOfWork.Companies.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.Companies.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
