...
namespace ExpressDesk360.Business.Concrete
{
    public class InvoiceTypeService : IInvoiceTypeService
    {
        ...
        {
            await _unitOfWork.InvoiceTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.InvoiceTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
