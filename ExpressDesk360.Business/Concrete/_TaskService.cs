...
{
    public class _TaskService : ITaskService
    {
        ...
        {
            var affected = await _unitOfWork._Tasks.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork._Tasks.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
