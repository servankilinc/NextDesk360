...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketMessageService : ITicketMessageService
    {
        ...
        {
            await _unitOfWork.TicketMessages.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.TicketMessages.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
