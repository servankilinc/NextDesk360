...
namespace ExpressDesk360.Business.Concrete
{
    public class FaultTypeService : IFaultTypeService
    {
        ...
        {
            var affected = await _unitOfWork.FaultTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.FaultTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
