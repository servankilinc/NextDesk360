...
namespace ExpressDesk360.Business.Concrete
{
    public class StockMovementTypeService : IStockMovementTypeService
    {
        ...
        {
            await _unitOfWork.StockMovementTypes.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockMovementTypes.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
