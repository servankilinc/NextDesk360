...
namespace ExpressDesk360.Business.Concrete
{
    public class StockGroupBrandMapService : IStockGroupBrandMapService
    {
        ...
        {
            await _unitOfWork.StockGroupBrandMaps.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockGroupBrandMaps.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
