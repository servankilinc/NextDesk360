...
namespace ExpressDesk360.Business.Concrete
{
    public class TicketStatusService : ITicketStatusService
    {
        ...
        {
            await _unitOfWork.TicketStatuses.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(int id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.TicketStatuses.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
