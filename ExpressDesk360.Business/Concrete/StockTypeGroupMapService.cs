...
namespace ExpressDesk360.Business.Concrete
{
    public class StockTypeGroupMapService : IStockTypeGroupMapService
    {
        ...
        {
            await _unitOfWork.StockTypeGroupMaps.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockTypeGroupMaps.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
