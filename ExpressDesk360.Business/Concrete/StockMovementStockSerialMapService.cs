...
namespace ExpressDesk360.Business.Concrete
{
    public class StockMovementStockSerialMapService : IStockMovementStockSerialMapService
    {
        ...
        {
            await _unitOfWork.StockMovementStockSerialMaps.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }

        public async Task<Result> RestoreAsync(Guid id, CancellationToken cancellationToken = default)
        {
            await _unitOfWork.StockMovementStockSerialMaps.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
            return Result.Success();
        }
