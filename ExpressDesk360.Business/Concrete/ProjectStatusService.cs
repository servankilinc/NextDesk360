...
namespace ExpressDesk360.Business.Concrete
{
    public class ProjectStatusService : IProjectStatusService
    {
        ...
        {
            var affected = await _unitOfWork.ProjectStatuses.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.ProjectStatuses.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
