...
namespace ExpressDesk360.Business.Concrete.StockModule
{
    public class StockSerialWarrantyService : IStockSerialWarrantyService
    {
        ...
        {
            var affected = await _unitOfWork.StockSerialWarranties.DeleteAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
        ...
        {
            var restored = await _unitOfWork.StockSerialWarranties.RestoreAndSaveAsync(where: (f) => f.Id == id, cancellationToken);
